...
app.post('/api/sendEMB', (req, res) => {
  try {
    ...
    console.log('🌐 TÌM FILE EMB TRONG MẠNG LAN:');
    ...
    console.log(`   🎯 Số file .emb tìm thấy: ${embFiles.length}`);
    ...
    if (embFiles.length > 1) {
      ...
      console.log('   📋 Danh sách file .emb:');
      embFiles.forEach((file, index) => {
    ...
    // Xóa tất cả file trong thư mục fileEMB trước khi copy
    console.log('🧹 XÓA TẤT CẢ FILE TRONG THƯ MỤC ĐÍCH:');
    const existingFiles = fs.readdirSync(fileEMBDir);
    console.log(`   📊 Số file hiện có: ${existingFiles.length}`);
    ...
    if (existingFiles.length > 0) {
      ...
      console.log('   ✅ Đã xóa tất cả file cũ');
    } else {
     
    }

    // Copy file với kiểm tra lỗi
    console.log('📋 BẮT ĐẦU COPY FILE:');
    console.log(`   📂 Từ: ${sourcePath}`);
    console.log(`   🎯 Đến: ${targetPath}`);
    ...
    try {
      ...
      console.log(`   📏 Kích thước: ${(fileStats.size / 1024).toFixed(2)} KB`);
      console.log(`   🕐 Thời gian: ${new Date().toISOString()}`);

      // Chạy file xuLyEMB.exe sau khi copy thành công
      console.log('🚀 CHẠY FILE XỬ LÝ EMB:');
      const exePath = path.join('C:', 'Users', currentUser, 'Desktop', 'serverEMB', 'xulyEMB_autoIT', 'xuLyEMB.exe');
      
      if (fs.existsSync(exePath)) {
        console.log(`   📂 Đường dẫn: ${exePath}`);
      ...
      console.error('❌ LỖI COPY FILE:');
      console.error(`   💥 Lỗi: ${copyError.message}`);
    ...
    console.log('✅ FILE EMB ĐÃ ĐƯỢC COPY THÀNH CÔNG:');


    // Đọc nội dung file để preview
    console.log("da nhan duocj 1 file emb");
  } catch (error) {
